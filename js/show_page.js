...
function save_edit() {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      window.location = `/hadfun&netflix-project/edit_post.php?post_id=${post_id.innerHTML}&post_content=${user_edit_post.value}`;
    } else {
...
function startSwal(post_id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location = `/hadfun&netflix-project/delete_post.php?post=${post_id}`;
    }
  });
}
